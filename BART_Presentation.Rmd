...
Larger values of $\alpha$ increase the likelihood of a split, and larger values of $\beta$ decrease the number of terminal nodes.
...
<div>
\[
\alpha(T_j, T_j^*)=\min\left\{1, \frac{q(T_j^*,T_j)}{q(T_j,T_j^*)} \frac{P(\mathbf{R}_j\mid \mathbf{x},T_j^*,M_j)}{P(\mathbf{R}_j\mid \mathbf{x},T_j,M_j)} \frac{P(T_j^*)}{P(T_j)} \right\}
\]
</div>
...
<div>
\[
N\left(\left(\sigma_\mu^2\sum r_{ji}\right)/(n_i\sigma_\mu^2+\sigma^2),\,\sigma^2\sigma_\mu^2 /[n_i\sigma_\mu^2+\sigma^2]\right)
\]
</div>
...
BART is often initialized with $m$ stumps, with $\mu_{j1}=\frac{\bar{Y}}{m}$.
...
Assuming we've chosen a suitable $K$, the sequence of iterations can be regarded as a sample from the true posterior. 

Burn-in iterations are only used to start the MCMC process, and allow the sampler to reach the true posterior region.
...
- You really care about computational cost
- You don't want to learn any Bayesian statistics to understand the model
...
There are two functions to fit a BART model in dbarts
```{r, eval = F}
...
- https://cran.r-project.org/web/packages/dbarts/dbarts.pdf for further reading
...
for (col in zero_cols) {
  diabetesCSV[[col]][diabetesCSV[[col]] == 0] <- NA
...
Chipman, Hugh A., et al. “Bart: Bayesian additive regression trees.” The Annals of Applied Statistics, vol. 4, no. 1, 1 Mar. 2010, https://doi.org/10.1214/09-aoas285. 

Dorie V, Chipman H, McCulloch R (2026). _dbarts: Discrete Bayesian Additive Regression Trees
Sampler_. doi:10.32614/CRAN.package.dbarts <https://doi.org/10.32614/CRAN.package.dbarts>, R package
version 0.9-33, <https://CRAN.R-project.org/package=dbarts>. 

Fan, Zhou. “Lecture 20 — Bayesian Analysis.” STATS 200: Introduction to Statistical Inference, Autumn 2016. https://web.stanford.edu/class/stats200/Lecture20.pdf

Hill, Jennifer, et al. “Bayesian additive regression trees: A review and look forward.” Annual Review of Statistics and Its Application, vol. 7, no. 1, 9 Mar. 2020, pp. 251–278, https://doi.org/10.1146/annurev-statistics-031219-041110. 

Lopes, Hedibert F., and Paulo Marques. “BART: Bayesian Additive Regression Trees.” Insper Institute of Education and Research São Paulo, Brazil, https://hedibert.org/wp-content/uploads/2018/06/BART.pdf
...
Sparapani R, Spanbauer C, McCulloch R (2021). “Nonparametric Machine Learning and Efficient
Computation with Bayesian Additive Regression Trees: The BART R Package.” _Journal of Statistical
Software_, *97*(1), 1-66. doi:10.18637/jss.v097.i01 <https://doi.org/10.18637/jss.v097.i01>.

Tan YV, Roy J. Bayesian additive regression trees and the General BART model.
Statistics in Medicine. 2019;38:5048–5069. https://doi.org/10.1002/sim.8347

Wang, Hui. “Chapter 5. Bayesian Statistics.” AM 166: Statistical Inference (II). https://www.dam.brown.edu/people/huiwang/classes/am166/Ch5.pdf
