...
---
#### More info to add on this package, will likely add another slide for it
## The  **dbarts** Package
...
- dbarts has a predict() function similar to other libraries. 
...
---
### Need to add title and format slide as well as comment code still
...
---
### We can use ggplot to compare accuracy as shown below
...
metrics_list <- list(BART = get_metrics(bart_table), GLM = get_metrics(glm_table))

comparison_df <- data.frame(
  Metric = c("Accuracy", "Precision", "True Positive Rate (Recall)"),
...
In this case, the following questions looks to be answered: **Can the given CTG Metrics correctly classify is a given fetus is at high risk (Health Score of 2 or 3)?** 
...
dictionaryDF <- data.frame(
  Variable = c("Accelerations", "Baseline Value", "Fetal Movement", "Decelerations", "Uterine Contractions", "Health Score"),
  Description   = c("Number of Times Heartrate is 15 BPM Over Baseline Value per Second", "Baseline Heartrate Level", "Number of Distinct Fetal Movements per Second", "Number of Times Heartrate is 15 BPM Under Baseline Value per Second", "Number of Uterine Contractions per Second", "Score Representing the current Health Status of a Fetus"),
  Type = c("Float", "Integer", "Integer", "Integer", "Float", "Integer (1, 2, or 3)") 
)
...
To begin the dataset needs to be cleaned of missing values, followed by assigning our predictor and response variables. After, the dataset is split into training/testing sets via random sampling.
...
classX = model.matrix(~ baseline.value + accelerations + fetal_movement + severe_decelerations + 
                        uterine_contractions, data = fetalCSV)
classY = fetalCSV$Target

#Training/Testing Splits
set.seed(1)

train_indices = sample(1:nrow(classX), 0.7 * nrow(classX))

trainX = classX[train_indices, ]
trainY = classY[train_indices]
testX  = classX[-train_indices, ]
testY  = classY[-train_indices]
```
...
Now, the BART model can be run on the data.

```{r, results='hide'}
bart_model = bart2(trainX, trainY, testX, keepTrees = TRUE, verbose = FALSE)
```

FIND SOMETHING ELSE FOR HERE

---

ADD IN STUFF SHOWING BART'S METRICS, COMBINE THE PLOTS YOU HAVE WITH BABY EXAMPLE METRICS
