...
where $\frac{q(T_j^*,T_j)}{q(T_j,T_j^*)}$ is ratio of the probability of how $T_j$ moves to $T_j^*$ to the probability of how $T_j^*$ moves to $T_j$, $\frac{P(\mathbf{R}_j\mid \mathbf{x},T_j^*,M_j)}{P(\mathbf{R}_j\mid \mathbf{x},T_j,M_j)}$ is the likelihood ratio of $T_j^*$ to that of $T_j$, and $\frac{P(T_j^*)}{P(T_j)}$ is the probability of $T_j^*$ to that of $T_j$. The exact formulas used in these calculations vary based on the chosen move, and have been omitted for sake of space.
...
3. Calculate probability $T_3^*$ is selected
...
- An early iteration such as this one would likely be part of the "burn-in" iterations, and would not be included in the final posterior that is used in predictions
]
...
- highlight that BART functions as a nonparametric Bayesian approach (it's a probability estimator)
- use proper terminology (posteriors, prior, weak learners, MCMC, etc)
- highlight the regularization prior to keep each tree as a weak learner: even without tuning, bart's default priors prevent it from overfitting the small iris dataset unlike glm which can be more sensitive to outliers
- understand how dbarts package handles classification 
- Quantitative results: Bart performs very well without the need for tuning (look into whether that is due to overfitting)
