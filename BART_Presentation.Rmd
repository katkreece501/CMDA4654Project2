...
- As more data is observed, the posterior distribution moves further from the prior
...
- A prior is conjugate for the likelihood if the posterior has the same distribution type of the prior. This typically makes calculations easier
...
We can calculate a $1-\alpha$ posterior for our posterior distribution $f_{P|H}(p\mid h)$ as the interval $[p_1, p_2]$ for which

$$
P(p<p_1)=\int_{p<p_1}f_{P|H}(p\mid h)\ dp=\frac{\alpha}{2}\text{ and }P(p>p_2)=\int_{p>p_2}f_{P|H}(p\mid h)\ dp=\frac{\alpha}{2}
$$
...
For higher values of $m$ (more trees), the prior on individual parameters tightens, ensuring predictions don't scale with $m$. Note that if you choose to scale $Y$, no scaling of any predictors are needed.
