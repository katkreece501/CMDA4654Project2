...
author:
  ...
  - "Ashley Powell, Samika Reddy, Kathleen Reece"
date: '`r Sys.Date()`'
output:
...
Suppose we want to make inference for some $Y$ that depends on the unknown function

$$
Y=f(\mathbf{x})+\epsilon,\quad \epsilon\in N(0,\sigma^2)
$$
...
$$
f(\mathbf{x}) \approx \sum_{j=1}^m g_j(\mathbf{x}) +\epsilon,\quad \epsilon\in N(0,\sigma^2) 
$$
...
- Easily incorporate interaction effects between terms

compared to more traditional regression or regression-like model.
...
- Introduction to Sum of trees
- Overview of other tree-based methods done in class or in wide use (Slide after central problem, but before introduction of BART
- Basic overview of BART (hit the basics, expand on later slides)
- Brief intro to Bayesian priors and posteriors
- Easy example of priors and posteriors 
